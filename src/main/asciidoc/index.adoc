...
``oc get sa default -n $namespace -o json | jq -r ".secrets[].name" | grep -v docker | xargs -i oc get secret {} -o json | jq --arg name "token" '.data[$name]' -r | base64 -d > /tmp/clerk-token``
